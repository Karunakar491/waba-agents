...
# Knowledge index — root
chars — when a purpose needs a paragraph, the paragraph belongs in the file, in
One line per file: `path — purpose`. **Grep this. Never read it whole.**
`TASKS.md`, or in `wiki/` — not here.
.claude/skills/devops-skill.md — DevOps persona (NEW 2026-08-07) — required gate for infra/deploy/migration/security tasks, after EL, never batched with EL; enforces CLAUDE.md's Kill Switch (feature flag, rollback, …
.claude/skills/persona-design-evaluator.md — Design Evaluator persona — world-class outside eye, runs cold after UX gate on new screens/DESIGN.md/token changes; BLOCKs only on generic AI-generated feel; model claude-fable-5 (2026-08-07)
.claude/skills/persona-el.md — Engineering Lead persona — code, debug, PR review, implementation standards
.claude/skills/persona-em.md — Engineering Manager persona — architecture, tech decisions, delivery risk
.claude/skills/persona-pm.md — Head of Product persona (rewritten 2026-07-22) — 20yr SaaS/ex-Salesforce visionary voice; holds conviction across full discussion, revisits unresolved threads unprompted, does not fold when user …
.claude/skills/persona-ux.md — UI/UX Designer persona gate — whole-app-context review, DESIGN.md S5 checklist step 0, token single-source enforcement, P0 flow UX checklist
.claude/skills/QA.md — QA agent persona (2026-08-07) — advisory gate after EL APPROVE + automated CI, every code task; correctness/completeness/edge-case/security/perf/DESIGN.md/test-quality checklist; never blocks …
...
AGENT-WORKFLOW.md — Maker-checker flow, model assignments, gate batching, escalation — frontend sequence UX -> Design Evaluator -> EL (EL always final)
